...
export function AiGradePanel({ attemptId }: { attemptId: string }) {
  ...
  return <section className="mt-5 space-y-3 border-t border-border pt-5">
    ...
    {grade ? <div><span className={`rounded-full px-2 py-1 text-xs ${calibrated ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-900'}`}>{calibrated ? 'Household calibrated' : 'Provisional — not yet calibrated'}</span><p className="mt-2 text-2xl font-semibold">{grade.total_score} / 5</p></div> : configured ? <button type="button" onClick={run} disabled={loading} className="rounded-xl border border-accent px-4 py-2 text-accent">{loading ? 'Grading…' : `Grade with AI · est. $${estimatedGradeCostUsd.toFixed(4)}`}</button> : <p className="text-sm text-ink-muted">No provider is configured. Self and partner grading remain free and fully available.</p>}
