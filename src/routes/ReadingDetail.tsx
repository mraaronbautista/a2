...
export function ReadingDetail() {
  ...
  return <div ref={rootRef} className="relative flex h-full min-h-0 flex-col overflow-hidden bg-bg">
    <ReaderToolbar courseId={course.id} title={reading.title} courseName={course.name} sidebarOpen={sidebarOpen} viewMode={progress.viewMode} bookmarked={bookmarked} rotation={rotation} inkActive={inkActive} onInk={() => setInkActive((value) => !value)} onToggleSidebar={() => setSidebarOpen((value) => !value)} onViewMode={progress.setViewMode} onBookmark={() => state.toggleBookmark(progress.page)} onNote={() => setLinkedNoteOpen(true)} onPrint={print} onDownload={download} onFullscreen={() => rootRef.current?.requestFullscreen()} onRotate={() => setRotation((value) => (value + 90) % 360)} />
    ...
    <div className="relative flex min-h-0 flex-1">
      ...
      {progress.viewMode === 'page' ? <PdfDocument pdf={pdfState.document} page={progress.page} zoomMode={progress.zoomMode} zoom={progress.zoomValue} rotation={rotation} annotations={state.annotations} inkActive={inkActive} onInk={(anchor,color) => void state.createInk(progress.page,anchor,color)} onTextSelection={(text, anchor) => setSelection({ text, anchor })} /> : <ReflowView pages={reflowPages} loading={reflowLoading} onOpenPage={(page) => { progress.setPage(page); progress.setViewMode('page') }} />}
    </div>
